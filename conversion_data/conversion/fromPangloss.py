            ...
            if b_root == False:
                ...
                for key, value in root.items():
                    if key.endswith("lang"):
                        g_lang = key
                        trans.metadata.transcript.open["languages"] = [value]
                    elif key == "id":
                ...
                continue
                # Notes
            elif ((elem.tag == "NOTE") or (elem.tag == "AREA") or
                  (elem.tag == "FOREIGN")):
                continue
                # Levels
            else:
            ...
            if header == False:
                ...
                if soundfile:
                    ...
                    if "\\" in u:
                        ...
                        n = u
                    trans.metadata.recording.name.append(n)
                    trans.metadata.recording.type.append("")
                trans.metadata.header = "\t" + (ETree.tostring(elem, "utf-8")
                                                     .decode("utf-8"))
                header = True; root.remove(elem)
                # End of tier
            elif (elem.tag == "FORM") or (elem.tag == "TRANSL"):
                tier.segments[-1].content = escape(elem.text)
