    ...
    for tier in trans:
        ...
        while b < len_seg:
            ...
            if (p_start + 1) == p_end:
                if content:
                    content = content #+ "_U"
                n_tier.addsegment(start, end, content, seg.id,
                ...
                while p_mid < p_end:
                    ...
                    if p_mid == p_start:
                        ...
                        if content:
                            content = content #+ "_I"
                    n_tier.addsegment(n_start, n_end, content, seg.id,
                                      seg.ref, seg.prev, seg.unit)
    ...
    for tier in ntrans:
        ...
        for seg in tier:
            ...
            if max <= 0:
                if seg.prev != "":
                    prev = n_tier.segments[-1].id
                n_tier.addsegment(seg.start, seg.end, seg.content, 
                                  seg.id, seg.ref, prev, seg.unit)
            else:
                for c in range(0, max):
                    ...
                    if seg.prev != "":
                        prev = n_tier.segments[-1].id
                    n_tier.addsegment(start, end, seg.content, seg.id,
                                      seg.ref, prev, seg.unit)
        tier.segments.clear()
        for seg in n_tier.segments:
            tier.addsegment(seg.start, seg.end, seg.content,
